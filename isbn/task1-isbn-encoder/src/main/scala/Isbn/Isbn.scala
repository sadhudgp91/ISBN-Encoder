...
object Isbn
{
  ...
  def explodeIsbn():DataFrame = {
    ...
    val rdd1 = sc.parallelize(Seq(("Learning Spark: Lightning-Fast Big Data Analysis", 2015, Seq("ISBN: 978-1449358624",ean,grp,pub,tit))))
  ...
  def ISBNEncoder(){
    ...
    df.createOrReplaceTempView("IsbnTable")
